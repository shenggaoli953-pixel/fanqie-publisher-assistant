...
1. 在“小说排程”添加作品文件夹，确认识别到的章节、起始日期、发布范围和每日限制。
2. “发布时间”可以填写一个或多个时间，例如 `08:00, 12:00, 20:00`。同一时段有多章时会按分钟保持顺序。
3. 点击“查看发布清单”核对每章的标题、字数、时间和状态；清单不展示正文。
4. 点击“发布排程”后，工具只在开始时同步一次后台状态，然后按排程连续提交；平台提示需要人工处理时会停止，不会跳过失败章节。
5. 若有真实失败，选择“从失败处继续”。需要排查时可导出不含正文和账号信息的诊断文件。
...
新版本会通过 [Releases](https://github.com/shenggaoli953-pixel/fanqie-publisher-assistant/releases) 发布。反馈问题时请附上软件版本、操作步骤和导出的诊断文件；不要发送账号、验证码、Cookie、正文或 Edge 配置目录。
